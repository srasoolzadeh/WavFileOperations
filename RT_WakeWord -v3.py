...
#---------------------
data = np.zeros(441000, dtype='int16')
fs, wavdata = wavfile.read('train.wav')  
wave1=wavdata.swapaxes(0,1) 
r1=np.array(wave1[0,:])
write('test2.wav', 44100, r1)

#data0 = np.array(r1[0:22050])
#data = np.append(data, data0)
#data = data[22050:]
#
#spectrum = get_spectrogram(data) # calculates fft for windows of size 200 , each 80 samples shift forces new fft
## output : ( 101, 5511) for data[441000]  AND (101, 998) for data[80000]
#preds = detect_triggerword_spectrum(spectrum) # converts (101, 5511) => (1375,) output of the trained model
#new_trigger = has_new_triggerword(preds, chunk_duration, feed_duration)


for i in range(20):
    data0 = np.array(r1[i*22050:(i+1)*22050])
    data = np.append(data, data0)
    data = data[22050:]
    spectrum = get_spectrogram(data) # calculates fft for windows of size 200 , each 80 samples shift forces new fft
    # output : ( 101, 5511) for data[441000]  AND (101, 998) for data[80000]
